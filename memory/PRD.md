...
- ✅ **🤖 AI Dev tab (2026-06-04)** — admin-only conversational coding agent. New `/api/admin/agent/chat` endpoint runs a multi-step tool-calling loop (max 8 steps): the LLM emits `{"tool":"...", ...}` JSON action blocks (read_file, list_files, propose_edit, apply_last_edit, github_push, done). Frontend renders the chat thread with collapsible per-message tool traces. Pending drafts surface a banner with ✅ Apply / ✗ Discard buttons; admin can flip `⚡ Auto-apply` to skip the approval click for trusted edits.
- ✅ **🧑 Humanizer in Write tab (2026-06-04)** — toggle that (a) adds a HUMANIZE-MODE instruction to the /write system prompt (contractions, varied sentence length, banned AI-ese phrases) and (b) makes the ghost-typer occasionally mistype a neighboring keyboard letter, pause, backspace, and correct — exactly like a human typing. Includes longer "think pauses" at random and more delay variance at sentence boundaries.
